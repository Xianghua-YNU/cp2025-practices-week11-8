...
def trapezoid_integration(length, z, n_points=100):
    """使用梯形积分法计算二重积分"""
    ...
    for i in range(n_points):
        for j in range(n_points):
            ...
            if j == 0 or j == n_points - 1:
                ...
                wy = dy
            integral += wx * wy * integrand(x[i], y[j], z)
    return integral
...
def calculate_force(length, mass, z, method='gauss'):
    ...
    if method == 'gauss':
        ...
        integral = trapezoid_integration(length, z)
...
def plot_force_vs_height(length, mass, z_min=0.1, z_max=10, n_points=100):
    ...
    # Calculate force using both methods
    F_gauss = [calculate_force(length, mass, z, method='gauss') for z in z_values]
    F_trapezoid = [calculate_force(length, mass, z, method='trapezoid') for z in z_values]
    
    # Plot results
    plt.figure(figsize=(10, 6))
    plt.plot(z_values, F_gauss, 'r-', label='Gauss-Legendre')
    plt.plot(z_values, F_trapezoid, 'g:', label='Trapezoid Integration')
...
def compare_integration_methods(length, mass, z_values):
    """Compare Gauss-Legendre and trapezoid integration methods"""
    results = []
    for z in z_values:
        # Calculate using both methods
        gauss_result = calculate_force(length, mass, z, method='gauss')
        trapezoid_result = calculate_force(length, mass, z, method='trapezoid')
        
        # Calculate relative difference
        diff = abs(gauss_result - trapezoid_result)
        rel_diff = diff / trapezoid_result if trapezoid_result != 0 else 0
    ...
    print("-" * 80)
    print(f"{'z (m)':<10}{'Gauss (N)':<20}{'Trapezoid (N)':<20}{'Diff':<15}{'Rel Diff':<15}")
    print("-" * 80)
    for r in results:
        print(f"{r['z']:<10.3f}{r['gauss']:<20.6e}{r['trapezoid']:<20.6e}"
